...
class ReportController extends Controller
{
    ...
    // 🆕 UPDATED: Upload method with document type support (COMPLETE METHOD)
...
Log::info('🔍 Upload request received', [
    ...
    'file_mime' => $request->hasFile('file') ? $request->file('file')->getMimeType() : 'none',
    'file_size' => $request->hasFile('file') ? $request->file('file')->getSize() : 0,
]);

// 🔧 TEMPORARY FIX: Allow all file types while debugging
$request->validate([
    'file' => 'required|file|max:51200', // 50MB max, no MIME restriction
    ...
    'document_type' => 'nullable|string'
        ...
        // 🆕 NEW: Get document type with safe default (BACKWARD COMPATIBLE)
        $documentType = $request->input('document_type', PatientReport::DOCUMENT_TYPE_LAB_REPORT);
        ...
        // 🔍 DEBUG: Log file extension detection
        Log::info('🔍 File extension detection', [
        ...
        Log::info('Report upload started with document type', [
            'patient_id' => $patientId,
            'file_type' => $ext,
            'document_type' => $documentType, // 🆕 NEW: Log document type
            'file_size' => $file->getSize(),
            'is_image' => $isImage,
        ...
        try {
            ...
            if ($isImage) {
                ...
                // 🆕 NEW: OCR status based on document type
                $ocrStatus = $this->getOCRStatusForDocumentType($documentType);
            ...
            // Step 2: Create report record with document type
            $report = PatientReport::create([
                ...
                'document_type' => $documentType, // 🆕 NEW: Save document type
                'notes' => $request->notes,
                'report_date' => $request->report_date ?? now(),
                'report_title' => $request->report_title ?? $this->getDefaultTitle($documentType),
                'uploaded_by' => 'patient',
                'ocr_status' => $ocrStatus,
                'processing_attempts' => 0
            ]);

            // Step 3: Process file for text extraction (CONDITIONAL BASED ON DOCUMENT TYPE)
            $extractedText = '';
            $createdMetrics = [];
            
            if ($isPdf) {
                // Extract text from PDF
                Log::info('🚀 Starting PDF text extraction', ['report_id' => $report->id, 'document_type' => $documentType]);
            ...
            } else if ($isImage && $report->requiresOCRProcessing()) {
                // Process image with OCR (only for certain document types)
            ...
            // Step 4: Generate AI summary ONLY for lab reports (CONDITIONAL AI PROCESSING)
            $aiSummaryJson = null;
            $cleanedText = '';
            
            if ($report->requiresAIProcessing() && !empty($extractedText)) {
                Log::info('🤖 Starting AI summary generation for lab report', [
                    'report_id' => $report->id,
                    'document_type' => $documentType,
                    'text_length' => strlen($extractedText)
                ]);
            ...
            } else if (!$report->requiresAIProcessing()) {
                Log::info('⏭️ Skipping AI processing for document type', [
                    'report_id' => $report->id,
                    'document_type' => $documentType,
                    'reason' => 'Document type does not require AI processing'
                ]);
                
                // For non-AI documents, still save OCR text if available
                $cleanedText = $this->cleanTextForDatabase($extractedText);
            }

            // Create AI summary record (even for non-AI documents to maintain structure)
            $aiSummary = AISummary::create([
                ...
                'confidence_score' => isset($aiSummaryJson['confidence_score']) 
                    ...
                    : 0,
                'ai_model_used' => $aiSummaryJson ? 'gpt-4' : null,
            ]);

            // Step 5: ✨ SAFE HEALTH METRICS EXTRACTION (ONLY FOR LAB REPORTS)
            if ($report->requiresAIProcessing() && !empty($aiSummaryJson) && isset($aiSummaryJson['key_findings'])) {
                Log::info('🔬 Starting health metrics extraction for lab report', [
                    'report_id' => $report->id,
                    'document_type' => $documentType,
            ...
            // Step 7: Enhanced response with document type context
            $response = [
                'message' => $this->getSuccessMessage($documentType),
                'report_id' => $report->id,
                'file_type' => $report->type,
                'document_type' => $documentType, // 🆕 NEW: Include document type in response
                'document_type_config' => $report->getDocumentTypeConfig(), // 🆕 NEW: Include type config
                'ocr_status' => $report->ocr_status,
                'ai_processing_enabled' => $report->requiresAIProcessing(), // 🆕 NEW: Indicate if AI was used
                'raw_text_preview' => Str::limit($extractedText, 300),
                'summary' => $aiSummaryJson,
                'confidence_score' => $aiSummaryJson['confidence_score'] ?? null,
                
                // ✨ Enhanced health metrics information (only for lab reports)
                'health_metrics' => $report->requiresAIProcessing() ? $this->createDetailedMetricSummary($createdMetrics) : null,
            ...
            Log::info('🎉 Report upload completed successfully', [
                'report_id' => $report->id,
                'document_type' => $documentType,
                'ai_processing_used' => $report->requiresAIProcessing(),
                'ocr_status' => $report->ocr_status,
    ...
    /**
     * 🆕 NEW: Get OCR status based on document type
     */
    private function getOCRStatusForDocumentType($documentType)
    {
        $ocrDocumentTypes = [
            PatientReport::DOCUMENT_TYPE_LAB_REPORT,
            PatientReport::DOCUMENT_TYPE_PRESCRIPTION,
            PatientReport::DOCUMENT_TYPE_DISCHARGE_SUMMARY,
            PatientReport::DOCUMENT_TYPE_OTHER
        ];

        return in_array($documentType, $ocrDocumentTypes) 
            ? PatientReport::OCR_STATUS_PENDING 
            : PatientReport::OCR_STATUS_NOT_REQUIRED;
    }

    /**
     * 🆕 NEW: Get default title based on document type
     */
    private function getDefaultTitle($documentType)
    {
        $defaultTitles = [
            PatientReport::DOCUMENT_TYPE_LAB_REPORT => 'Lab Test Report',
            PatientReport::DOCUMENT_TYPE_PRESCRIPTION => 'Prescription',
            PatientReport::DOCUMENT_TYPE_DISCHARGE_SUMMARY => 'Hospital Discharge Summary',
            PatientReport::DOCUMENT_TYPE_VACCINE_CERTIFICATE => 'Vaccination Certificate',
            PatientReport::DOCUMENT_TYPE_INSURANCE => 'Health Insurance Document',
            PatientReport::DOCUMENT_TYPE_SCAN => 'Medical Scan',
            PatientReport::DOCUMENT_TYPE_OTHER => 'Medical Document'
        ];

        $baseTitle = $defaultTitles[$documentType] ?? 'Medical Document';
        return $baseTitle . ' - ' . now()->format('M Y');
    }

    /**
     * 🆕 NEW: Get success message based on document type
     */
    private function getSuccessMessage($documentType)
    {
        $messages = [
            PatientReport::DOCUMENT_TYPE_LAB_REPORT => 'Lab test report uploaded and analyzed successfully.',
            PatientReport::DOCUMENT_TYPE_PRESCRIPTION => 'Prescription uploaded successfully.',
            PatientReport::DOCUMENT_TYPE_DISCHARGE_SUMMARY => 'Discharge summary uploaded successfully.',
            PatientReport::DOCUMENT_TYPE_VACCINE_CERTIFICATE => 'Vaccine certificate uploaded successfully.',
            PatientReport::DOCUMENT_TYPE_INSURANCE => 'Insurance document uploaded successfully.',
            PatientReport::DOCUMENT_TYPE_SCAN => 'Medical scan uploaded successfully.',
            PatientReport::DOCUMENT_TYPE_OTHER => 'Medical document uploaded successfully.'
        ];

        return $messages[$documentType] ?? 'Document uploaded successfully.';
    }
    ...
    private function processImageWithOCR(PatientReport $report)
    {
        try {
            ...
            Log::error('OCR processing failed', [
                'report_id' => $report->id,
                'document_type' => $report->document_type ?? 'unknown',
                'error' => $e->getMessage()
            ]);
    ...
    public function retryOCR(Request $request, $id)
    {
        ...
        try {
            ...
            if ($ocrResult['success']) {
                ...
                // Re-run AI processing with the new text (only for lab reports)
                if (!empty($ocrResult['text']) && $report->requiresAIProcessing()) {
    ...
    public function getOCRStatus($id)
    {
        ...
        return response()->json([
            'report_id' => $report->id,
            'document_type' => $report->document_type, // 🆕 NEW: Include document type
            'ocr_info' => $report->getOCRStatusInfo(),
    ...
    /**
     * 🆕 UPDATED: Get all reports for the authenticated patient (with document type info)
     */
        ...
        $reports = PatientReport::with(['aiSummary'])
            ...
            ->map(function ($report) {
                return [
                    ...
                    'file_type' => $report->type,
                    'document_type' => $report->document_type, // 🆕 NEW: Include document type
                    'document_type_display' => $report->getDocumentTypeDisplayName(), // 🆕 NEW: Display name
                    'document_type_config' => $report->getDocumentTypeConfig(), // 🆕 NEW: Type configuration
                    'ai_processing_enabled' => $report->requiresAIProcessing(), // 🆕 NEW: AI status
    ...
    public function show($id)
    {
        ...
        // Get organ-grouped health metrics for this specific report (only for lab reports)
        $organGroupedMetrics = $report->requiresAIProcessing() 
            ? $this->getOrganGroupedMetricsForReport($report) 
            : [];
        
        // Get simplified findings with better explanations (only for lab reports)
        $simplifiedFindings = $report->requiresAIProcessing() 
            ? $this->getSimplifiedFindings($report) 
            : [];
        ...
        return response()->json([
            ...
            // ✨ NEW: Organ-grouped metrics for this report (only for lab reports)
            'organ_health' => $organGroupedMetrics,
            
            // ✨ NEW: Simplified findings with patient-friendly language (only for lab reports)
    ...
    public function getOrganInsights(Request $request, $id, $organKey)
    {
        try {
            ...
            // Only provide organ insights for lab reports
            if (!$report->requiresAIProcessing()) {
                return response()->json([
                    'error' => 'Organ insights are only available for lab test reports',
                    'document_type' => $report->document_type
                ], 400);
            }
    ...
    public function downloadFile($id)
    {
        try {
            ...
            \Log::info('✅ Report found', [
                ...
                'file_type' => $report->type,
                'document_type' => $report->document_type, // 🆕 NEW: Log document type
            ...
            // Generate download filename based on document type
            $extension = pathinfo($filePathToServe, PATHINFO_EXTENSION);
            $downloadName = ($report->report_title ?? $report->getDocumentTypeDisplayName()) . '.' . $extension;
    ...
    public function destroy($id)
    {
        ...
        return response()->json([
            'message' => $this->getDeleteSuccessMessage($report->document_type ?? 'lab_report') // 🆕 NEW: Context-aware delete message
        ]);
    }

    /**
     * 🆕 NEW: Get delete success message based on document type
     */
    private function getDeleteSuccessMessage($documentType)
    {
        $messages = [
            PatientReport::DOCUMENT_TYPE_LAB_REPORT => 'Lab test report deleted successfully.',
            PatientReport::DOCUMENT_TYPE_PRESCRIPTION => 'Prescription deleted successfully.',
            PatientReport::DOCUMENT_TYPE_DISCHARGE_SUMMARY => 'Discharge summary deleted successfully.',
            PatientReport::DOCUMENT_TYPE_VACCINE_CERTIFICATE => 'Vaccine certificate deleted successfully.',
            PatientReport::DOCUMENT_TYPE_INSURANCE => 'Insurance document deleted successfully.',
            PatientReport::DOCUMENT_TYPE_SCAN => 'Medical scan deleted successfully.',
            PatientReport::DOCUMENT_TYPE_OTHER => 'Medical document deleted successfully.'
        ];

        return $messages[$documentType] ?? 'Report deleted successfully.';
    }

    /**
     * Get detailed information for a specific finding (ONLY FOR LAB REPORTS)
     */
    public function getFindingDetails(Request $request, $id)
    {
        try {
            ...
            // 🆕 NEW: Only provide finding details for lab reports
            if (!$report->requiresAIProcessing()) {
                return response()->json([
                    'error' => 'Finding details are only available for lab test reports',
                    'document_type' => $report->document_type
                ], 400);
            }
    ...
    /**
     * Download AI summary as PDF (ONLY FOR LAB REPORTS)
     */
    public function downloadSummaryPdf($id)
    {
        ...
        // 🆕 NEW: Only provide PDF summary for lab reports
        if (!$report->requiresAIProcessing()) {
            return response()->json([
                'error' => 'AI summary PDF is only available for lab test reports',
                'document_type' => $report->document_type
            ], 400);
        }
